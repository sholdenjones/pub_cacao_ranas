...
# remove variables not used in PCA or plotting
active_data <- active_data[,-c(1,3:11,16)]
...
```{r}
pca_fit <- prcomp(
  x = active_data[, c(2:11)], # only use continuous variables
                  scale. = TRUE)
...
plot_pca <- fviz_pca_biplot(pca_fit,
                label = "var",
                habillage = active_data$"Site type",
...
##d. assess correlation of predictor variables


## #######nice histogram figure needs to go in here #######
...
```{r}
ggpairs(active_data, columns = c(17,18,19,20,21,22))
```
