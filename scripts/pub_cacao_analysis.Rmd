...
## NEED TO DELETE TWO SITES THAT WE DID NOT SAMPLE IN SPRING
...
1. NMDS to visualize community differences 
- follow the S.E.'s methods for this and cite him

You would typically:
Use a dissimilarity index like Bray-Curtis (good for abundance data).
Plot the NMDS and color-code points by site type.
Optionally, add convex hulls or ellipses for group visualization.
## COPY AND REFINE CODE


```{r}
# create a site type vector - this MUST align with order of sites
site_types <- c(rep("Sun cacao", 11),
                rep("Shade cacao", 12),
...
nmds_data$site_type <- factor(site_types,
                              levels = c("Sun cacao", "Shade cacao", 
...
plot_nmds_bray <- ggplot(nmds_data, aes(x = NMDS1, y = NMDS2,
                                        color = site_type)) +
  geom_point(size = 4) +
  scale_color_manual(values = c("Sun cacao" = "#DFC27D",
                                "Shade cacao" = "#A6611A",
...
2. then run PERMANOVA to quantify differences across land use types
- adonis, in vegan package - good for non-normal variable (?)
## NEW CODE

3. Look at sampling coverage for each land use type
- using inext package
## NEW CODE

4. then look at rank abundance curves across land-use types?
- using vegan - recommended by S.E. b/c dominance of priach
## NEW CODE

5. extrapolate communities using iNEXT - I think just do same thing as previouse code
## COPY CODE


6. compare abundance of species of conservation concern
- just need to copy and paste this code from previous analysis script
## COPY CODE
...
#5. GLMs for each predictor variable for active cacao plantations only
