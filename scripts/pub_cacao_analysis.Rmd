...
calculate mean and sd for each land use type
```{r}
type_environmental_summary <- site_data %>%
  group_by(Tipo) %>%
  summarise(
    across(all_of(vars),
...
elevation was not normally distributed
- run a Kruskal-Wallis test instead
```{r}
kruskal.test(Elevation ~ Tipo, data = site_data)
```
