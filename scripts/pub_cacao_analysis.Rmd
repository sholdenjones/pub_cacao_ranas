...
```{r}
new_matrix <- cbind(site_types, new_matrix)

species_by_habitat <- new_matrix %>%
  group_by(site_types) %>%
...
plot_rank_site <- ggplot(rac_long, aes(x = Rank, y = Abundance, color = site_types)) +
  ...
  labs(x = "Species Rank", 
       ...
       color = "Land use type",
       title = "Rank abundance curves by habitat type") +
  theme_minimal()
...
plot_rank_species
#ggsave("output/plot_rank_species.png", width =10, height = 10)
```
