...
# above will be used in manuscript, but create below w/ axes for presentations
plot_extrapolate_richness_full <- ggiNEXT(extrapolate_richness, type = 1) +
  ylab("Species richness") +
  scale_color_manual(values = c(
    "Sun cacao (N = 10)" = "#DFC27D", 
    "Shade cacao (N = 11)" = "#A6611A",
...
plot_extrapolate_richness_full
#ggsave("output/plot_extrapolate_richness_full.png", width = 15, height = 10)
```
...
# above will be used in manuscript, but create below w/ axes for presentations
plot_extrapolate_shannon_full <- ggiNEXT(extrapolate_shannon, type = 1) +
  ylab("Shannon diversity") +
  scale_color_manual(values = c(
    "Sun cacao (N = 10)" = "#DFC27D", 
    "Shade cacao (N = 11)" = "#A6611A",
...
plot_extrapolate_shannon_full
#ggsave("output/plot_extrapolate_shannon_full.png", width = 15, height = 10)
```
...
#ggsave("output/plot_extrapolate_simpson.png", width = 15, height = 10)
```
...
## calculate mean and sd for each tipo


```{r}
# Variables you want to analyze
vars <- c("site_mean_hoja",
          "site_mean_CC",
          "site_mean_veg",
          "forest_cover_400",
          "water_dist",
...
## this part isn't working, but see if can get for loop going here


```{r}
# summary_stats <- site_data %>%
#   select(Tipo, dplyr::all_of(vars)) %>% # specify all_of from dplyr
...
assess statistical significance of environmental variables across land use types

```{r}
# ensure Tipo is a factor
site_data$Tipo <- as.factor(site_data$Tipo)
```

use for loop to do the following for each environmental variable
- test normality of distribution
- if normal, run ANOVA, if not, stop
- if significant, run Tukey HSD, if not, stop

tested and confirmed this is correct for forest, vegetation, elevation
```{r}
...
for (v in vars) {
  ...
  form <- as.formula(paste(v, "~ Tipo"))
  
  # Fit ANOVA model
  model <- aov(form, data = site_data)
  ...
  # Check normality (p > 0.05 means OK)
  if (shapiro$p.value > 0.05) {
    cat("Normality OK → Running ANOVA\n")
    anova_out <- summary(model)
    print(anova_out)
    
    # Save ANOVA result
    results[[v]]$anova <- anova_out
    
    # If ANOVA significant, run Tukey
    p_anova <- anova_out[[1]]$`Pr(>F)`[1]
    
    if (p_anova < 0.05) {
      cat("ANOVA significant → Running Tukey HSD\n")
      tuk <- TukeyHSD(model)
      print(tuk)
      results[[v]]$tukey <- tuk
    } else {
      cat("ANOVA NOT significant → Skipping Tukey\n")
    }
    
  } else {
    cat("Normality NOT met → Skipping ANOVA and Tukey for this variable\n")
  }
}
```


#### need to run different test for elevation b/c not normal ####
...
# get the second best model to calculate pseudo-R2 as requested by subject editor
second_simpson_model <- get.models(simpson_model_set, subset = 2)[[1]]

summary(second_simpson_model)

# assess best full model fit - NOTE THAT DHARMA DOES NOT WORK WITH BETAREG MODELS
plot(best_simpson_model)
```

visualize effects of elevation
```{r}
# marginal effects for elevation_scaled
eff <- ggpredict(best_simpson_model, terms = "elevation_scaled")

# plot with ggplot2
plot(eff) +
  labs(x = "Elevation (scaled)",
    y = "Predicted Simpson diversity") +
  theme_minimal(base_size = 14)
```
